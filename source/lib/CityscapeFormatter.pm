...
printSewers formats details about streets around the city.
...
sub printSewers {
    ...
    my $sewerverb   = ( $city->{'sewers'}->{'opening'} == 1 )     ? "is" : "are";

    my $content = "There $sewerverb ".NO('opening', $city->{'sewers'}->{'opening'})." leading to $city->{'name'}'s sewer.\n";
