...
sub describe_establishment {
    ...
    $content .= '<span class="establishment" id="establishment'.$establishment->{'seed'}.'">';
    $content .= "The $establishment->{'name'} is ".A($establishment->{'size_description'}).", $establishment->{'condition'}-looking ".num2en($establishment->{'floors'})." floor <b>$establishment->{'type'}</b>. \n";
