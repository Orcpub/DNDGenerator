...
sub printDescription {
    ...
    $content.= $npc->{'firstnames'}." most valued possession is a $npc->{'possession'}, which $npc->{'possessiondetail'}. \n";
...
printStats formats details about the vital stats of the npc.
...
sub printStats {
    ...
    foreach my $stat ( sort keys %{ $npc->{'stats'} } ) {
        $content .= "<li>$stat: ".$npc->{$stat."_value"}.' '.$npc->{$stat."_description"}."</li>\n"
