...
=head2 generate_sewers

Generate details on the sewers
...
sub generate_sewers {
    ...
    $city->{'sewers'}->{'content'} = parse_object( $city_data->{'sewers'} )->{'content'}  if ( !defined $city->{'sewers'}->{'content'} );

    $city->{'sewers'}->{'opening'} = d(4) + $city->{'sewers'}->{'opening'} if ( !defined $city->{'sewers'}->{'opening'} );
...
sub generate_establishments {
    ...
    $city->{'establishment_count'} = 20 + floor( ($city->{'size_modifier'} + 5) * 1.2) if ( !defined $city->{'establishment_count'} );
