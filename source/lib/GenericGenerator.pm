...
sub generate_stats {
    ...
    foreach my $statname (keys %{ $xml->{'stats'} } ){
        ...
        $ds->{'stats'}->{$statname}=(d(6)+d(6)+d(6)+d(6)) if (!defined $ds->{'stats'}->{$statname} );
        
        #$dice->roll('4d6k3')  roll 4d6 keep highest 3
        #my $dice = Games::Dice::Roll20->new();
        #my $result = $dice->roll('4d6k3');
        #$ds->{'stats'}->{$statname}=$result if (!defined $ds->{'stats'}->{$statname} );
